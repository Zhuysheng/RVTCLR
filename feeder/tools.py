...
def shear(data_numpy, r=0.5): #base
...
def temperal_crop(data_numpy, temperal_padding_ratio=6):  #base
...
def gaus_noise(data_numpy, mean= 0, std=0.01, p=0.5): #gaus noise
    if random.random() < p:
...
class GaussianBlurConv(nn.Module):   #gaus blur
...
def axis_mask(data_numpy, p=0.5):  #channel mask
    am = Zero_out_axis()
    if random.random() < p:
...
def joint_mask(data_numpy):  #channel mask
    am = Zero_out_joints()
    return am(data_numpy)
